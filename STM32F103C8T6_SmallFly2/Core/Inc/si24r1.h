...
//-----------------------SI24R1配置-------------------------//
#define NRF_RF_CH       100					// RF射频通道 0~125
#define	NRF_ADDR_WIDTH  5                  // RF地址宽度 1~5byte
#define NRF_DATA_WIDTH  31                  // RF数据宽度 1~32byte


//-----------------------SI24R1引脚配置-------------------------//
//#define NRF_SPI_NUM						SPI1
...
#define NRF_CE(x) 	 	HAL_GPIO_WritePin(NF_CE_GPIO_Port,NF_CE_Pin	,x);
#define NRF_CSN(x) 		HAL_GPIO_WritePin(NF_CS_GPIO_Port,NF_CS_Pin	,x);
#define NRF_IRQ     	HAL_GPIO_ReadPin(NF_IRQ_GPIO_Port,NF_IRQ_Pin)			//读取NRF状态
...
struct remoto_data{		//遥控器四轴数据结构体
	int8 L_X;			//左X轴
	int8 L_Y;			//左Y轴
	int8 R_X;			//右X轴
	int8 R_Y;			//右Y轴
	uint8 Flag;			//数据更新FLAG
	uint8 control;		//特殊指令FLAG
	uint32 Last_Time;	//上次更新时间
};//AD数据已经过归一化处理 -100 - 100



extern SPI_HandleTypeDef hspi1;				//SPI1
extern struct remoto_data Remoto_data;		//遥控器数据
...
// 用户函数
void  NRF_Dev_Init(void);						//NRF初始化
void  NRF_Send_Packet(uint8*);					//NRF发送
uint8 NRF_Rece_Packet(uint8 *p,uint8 len);		//NRF查询接收
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin);			//回传函数重定义，GPIO中断接收
